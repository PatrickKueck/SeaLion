...
######################################################################################################
# fundamental updates
# v1.023:	- added heatmap output: 'pheatmap_filtered_Pquartets', 'pheatmap_singlespecies_support'
# 			- process-time improved for building of species-quartets of each clade-quartet analysis -> changed subroutine &generate_taxon_quartets; &quartet_building_and_p4_main
#			- process-time improved for identification of character-only sites -> new subroutine &check_site_states
# 			- solved bug in parameter check during menu usage
# v1.024:	- corrected terminal print: ''Building species-quartets and site-pattern check of clade-quartet'' in &generate_species_quartets
#			- bug fixed for latex table sumed pdf print in non-existintend path 'PDF/' if R print is deactivated
######################################################################################################
...
my	%value_of_scr_par	=	(
	...
	'sys_parameter'		=>	{
							...
							'outgroup'					=>	0							,	# SeaLion parameter: specified code of defined outgroup-clan in clan-infile
							...
							'pen_bin'					=>	'sealion1.024.pl'			,	# SeaLion parameter: actual penguin script name
							'ice_bin'					=>	'icebreaker.o'				,	# SeaLion parameter: name of the terminal-executable icebreaker tree-fishing script
							'mean'						=>	0							,	# SeaLion parameter: if defined '1', calculation and print of mean support results. Blocked if '0'
							...
							'trsearch'					=>	""							,	# SeaLion parameter: optional filename for tree support search of predefined rooted-clade-trees
	}	,
	...
	'author_info'		=>	{
							...
							'email'						=>	'p.kueck[at]leibniz-lib.de'	,	
...
my	%print_Rplot_quartets	=	(
	...
	'pbarplot_tree_difference_mclan'			=>	1 , # MQ7*	Barplot:		MAIN	UserVersion	non-adapted	R barplot of method related score distance of method related best tree to other best-three tree and other, optionally defined, trees
	...
	'pheatmap_filtered_Pquartets'				=>	1 , # Q8*	Heatmap			MAIN	UserVersion	adapted	heatmap of rejected species contributions in 4clan rejected quartets
	'pheatmap_singlespecies_support'			=>	1 , # T9*	Heatmap			MAIN	UserVersion	adapted	heatmap of single species support contributions per clade-quartet
...
my	%print_latex	=	(
	...
	'table_besttree_frequencies_quartets'		=>	1 , # LQ8*	Table:	MAIN	List of unfiltered and filtered best tree frequencies of analysed species-quartets
...
my	%textfilename_of_printcode = (	
	...
	'txtLQ8'	=> 'LQ8_SeaLion_clade-quartet_best-tree_frequencies_of_species-quartets.txt',
	'txtLQ9'	=> 'LQ9_SeaLion_clade-quartet_best-tree_frequencies_of_species-quartets_stats.txt',
...
for my $clan4 ( sort keys %hol_fp_qcal_of_4clan_code ){
	...
	#####################################################################################
	# print single quartet and average 4-clan tree support (nap, napr)           		#
	#####################################################################################
	&print_tsv_MQ2(
	...
	#####################################################################################
	# print best tree frequencies & calculate distribution statistics (nap, napr) 		#
	#####################################################################################
	&print_tsv_MQ9_LQ8(
		...
		\%textfilename_of_printcode							, # key1: textfile unique printcode (e.g. 'textA'); value: filename for print																										IN: defined; OUT: changed
		\%hoh_linedata_of_latexfilename_of_printcode		, # key1: Latex table unique printcode (e.g. 'latexC'); key2: *.tex filename; value: @list of assigned table lines																	IN: defined; OUT: changed
	) ;
	#####################################################################################
...
##################################################################################### added 15.04.25 #
# sampling all best three tres of single icebreaker analyses (independent of nap, napr, napv)
# key1: type_support (mean,median); 
# key2: newick-str; 
# value: counter
my	%seen_rctrees_of_ts ;
#####################################################################################
		...
		if ( $N_trees_total == $N_4clan_combinations * 3 ){
			...
			if ( $value_of_scr_par{sys_parameter}{$stype} == 1 ){
				...
				&treefishing_result_processing(
					...
					\%algorithm_def_methods							, # key1: methode type; key2: method code; value: integer																											IN: defined; OUT: unchanged
					####################### added 15.04.25
					\%seen_rctrees_of_ts							, # key1: average (mean|median); value: newick of best three trees																									IN: undefined; OUT: defined/changed
					#######################
				) ;
				######################################################################
			}
...
##################################################################################### added 15.04.25 #
# sampling global identified best-three tree support and
# other infile defined rooted-clade trees (latter if specified)
# table and plot print of tree corresponding support value differences
# and support corresponding tree rankings in single analyses
&rooted_clade_support_summary(

	\%subfolder_q										, # hashpath to output-type specific subdirs of single quartet-pattern analyses; key: output-type (e.g. 'pdf'); value: pathdir										IN: defined; OUT: unchanged
	\%hoh_tsvdata_of_tsvfilename_of_printcode			, # key1: R grafic unique printcode (e.g. 'metafileT'); key2: *.tsv filename; value: *tsv filename data addings (e.g. clanfilename, type of support...)				IN: defined; OUT: changed
	\%value_of_scr_par									, # key1: sealion user available parameter options; value: optional parameter setting																				IN: defined; OUT: unchanged
	\%seen_rctrees_of_ts								, # key1: average (mean|median); value: newick of best three trees																									IN: defined; OUT: unchanged
	\$clanfile											, # name of given clanfile																																			IN: defined; OUT: unchanged
) ;
######################################################################################################
...
sub build_quartet_alignment{
	...
	my	$seqlength			 =	length $href_red_seq_of_taxa->{$aref_qtaxa->[0]} ;
		$$sref_alignment	.=	"_p4_temp_qmsa.phy" ;
...
sub check_clan_infile{
	...
	open IN, "<$clan_infile" or die "\nCLADEFILE-ERROR: Cannot read ", $clan_infile, "!\n\n" ;
	####################################
	...
	while ( my $line = <IN> ){ chomp $line;
		...
		if ( $line =~ /^\w+(,\w+)+$/ ){
			...
			if ( $seen_clan_startletter{$startletter} ){
				...
				print "\nCLADEFILE-ERROR: Found start letter '".$startletter."' for different clades!\n\n" ;
			...
			for my $tax ( 1 .. @lineparts-1 ){
				...
				if	( $seen_tax{$lineparts[$tax]} ){	
					...
					print "\nCLADEFILE-ERROR: Defined sequence name ", $lineparts[$tax], " appears multiple times in cladefile!\n\n";
			...
			if	( $seen_clan_name{$clanname} ){	
				...
				print "\nCLADEFILE-ERROR: Defined clade ", $clanname, " appears multiple times in cladefile!\n\n";
	...
	if ( $N_clans <= 3 ){	
		...
		print "\nCLADEFILE-ERROR: Number of defined clades < 4!\n\n";
	...
	unless ( $href_hol_taxa_of_clan->{$$sref_outgroup_clan} ){
		...
		print "\nCLADEFILE-ERROR: Cannot finde defined outgroup clade: ", $$sref_outgroup_clan ,"!\n\n";
...
sub check_taxon_quartet_number{
	...
	if ( @$aref_taxon_quartets > $href_set_opt->{qlimit}{qnumber} ){
		...
		unless ( $href_set_opt->{sys_parameter}{request} == 1 ){
			...
			print	"\n\t---------------------------------------------------\n" ,
					"\n\tNOTE: N quartets > max. limit of allowed species-quartets\n\t" ,
					"for clade combination ", $$sref_clan4, "!\n\n\t"			,
					"N quartets: ", $nq, "\n\t" 								,
					"Max. limit: ", $href_set_opt->{qlimit}{qnumber}, "\n\n\t"	,
					"Press <q>	<enter>\tto Quit\n\t"							,
					"Press <r>	<enter>\tto Choose ", $href_set_opt->{qlimit}{qnumber} ," Species-Quartet(s) Randomly\n\t",
...
sub config_menu{
	...
	print	"\n",
			...
			"\n\tSupport Type:\t\t type\t-average int\t<enter>\t\t('"				, $href_value_of_scr_par->{menu_parameter}{average}					,"')",
			"\n"																	,
...
sub check_user_parameter{
	...
	for my $cmdline ( @commands ){
		...
		if 		( ( $command =~ /^m$/ 			 ) && ( $parameter ) ){ 
			...
			if 	( 	$parameter =~ /^\d+$/ 		 ) { $href_value_of_scr_par->{qlimit}{qlength}		 = $parameter }
			else								   { print "\n\tPARAMETER-ERROR: Unallowed '-", $command, "' parameter: ", $parameter, "!\n\n"; &help( \$command, \%$href_value_of_scr_par ) }
		}
		elsif 	( 	$command =~ /^M$/ 		 	 ) { print "\n\tPARAMETER-ERROR: Missing parameter for command '-", $command, "'!\n\n"; &help( \$command, \%$href_value_of_scr_par 		   ) }
			...
			if 	( ( $parameter =~ /^\d+$/ ) 	   && ( $parameter >= 1   ) ) { $href_value_of_scr_par->{qlimit}{qnumber}		 = $parameter }
			else													   { print "\n\tPARAMETER-ERROR: Unallowed '", $command, "' parameter: ", $parameter, "!\n\n"; &help( \$command, \%$href_value_of_scr_par  ) }
		}
	...
	if		( $href_value_of_scr_par->{sys_parameter}{clfile} ){
		...
		else{  print "\nPARAMETER-ERROR: clade-filename includes non-alphanumeric signs!\n\n" ; &help( \'p', \%$href_value_of_scr_par ) }
	}
	#######################
	
	#######################
	# if systestart defined check for missing cladefile and outgroup
	if ( $href_value_of_scr_par->{sys_parameter}{sysstart} == 1 ){ 
		
		unless ( $href_value_of_scr_par->{sys_parameter}{clfile} 			){ print "\nPARAMETER-ERROR: Missing clade-file!\n\n" ; &help( \'p', \%$href_value_of_scr_par ) }
		unless ( $href_value_of_scr_par->{sys_parameter}{outgroup} =~ /\w+/ ){ print "\nPARAMETER-ERROR: Missing or incorrectly defined outgroup ", $href_value_of_scr_par->{sys_parameter}{outgroup}, "!\n\n" ; &help( \'o', \%$href_value_of_scr_par ) }
	}
	##############################################
...
sub evaluation_otu_support_main{
	...
	for	my $clan4 ( sort keys %{$href_hoh_sup_of_clan4_of_calc_of_qtree_of_fc} ){
		...
		######################################################################
		# write data to tsv. table T9                                        # added 15.04.25
		######################################################################
		&print_tsv_T9(
...
sub evaluation_quartet_inv_var{
	...
	if 		( $href_invvar_inf_of_type->{tNv_obs} > 0 ){ $href_invvar_inf_of_type->{rNi_Nv_obs}	=	$href_invvar_inf_of_type->{tNi_obs} / $href_invvar_inf_of_type->{tNv_obs} }
	else	{ $href_invvar_inf_of_type->{rNi_Nv_obs} = 0 }
...
sub generate_species_quartets{
	...
	print "\n\t::Building quartets and site-pattern check of clade-quartet ", $$sref_clan_combination ,"...(may take a while)\n";
	...
	my	$nq_total		=	( @clantaxa1 * @clantaxa2 * @clantaxa3 * @clantaxa4 ) ;
	
	my	%seen_rejected_quartets ;
	my	$quartet_counter_print 	= 0 ; # counts the number of analysed quartets, with predefined steps defined for print out
	my 	$quartet_counter 		= 0	; # counts the number of acceptable quartets with N fully resoved sites > defined minimum threshold
	
	if ( $href_data_of_fileprop->{length} < $href_val_of_scr_par->{qlimit}{qlength} ){
		
		die "\nPARAMETER-ERROR: defined minimum of fully-resoved sites (".$href_val_of_scr_par->{qlimit}{qlength}.") > total number of sites  ".$href_data_of_fileprop->{length}."!\n\n" 
	}
	
	##############################################
	# If number of available quartets suceeds the maximum number of quartets defined to be analysed,
	# do a user request if quartets should be selected randomly from the main pool of available quartets until defined maximum is reached
	my $answer	=	1 ;
	if ( $nq_total > $href_val_of_scr_par->{qlimit}{qnumber} ){
		
		#######################
		# ... user request (unless blocked by -u option)
		unless ( $href_val_of_scr_par->{sys_parameter}{request} == 1 ){

			print	"\n\t---------------------------------------------------\n" ,
					"\n\tNOTE: N quartets > max. limit of allowed species-quartets\n\t" ,
					"for clade combination ", $$sref_clan_combination, "!\n\n\t"			,
					"N quartets: ", $nq_total, "\n\t" 								,
					"Max. limit: ", $href_val_of_scr_par->{qlimit}{qnumber}, "\n\n\t"	,
					"Press <q>	<enter>\tto Quit\n\t"							,
					"Press <r>	<enter>\tto Choose ", $href_val_of_scr_par->{qlimit}{qnumber} ," Species-Quartet(s) Randomly\n\t",
					"Press		<enter>\tto Proceed\n\n\tCommand: "				;
					#"\n\t---------------------------------------------------\n" ;
			...
			if		( $answer =~ /q/i )	{ print "\n\tQuit ", $href_val_of_scr_par->{sys_parameter}{pen_bin} ,"!\n\n"; exit }
			#######################
		}
		#######################
		...
		# if user choose random sampling up to Max. Limit Nq
		if	(	(	$answer											=~	/r/i	)	||
				(	$href_val_of_scr_par->{sys_parameter}{request}	==	1		)	){

			print "\n\t", $href_val_of_scr_par->{qlimit}{qnumber} ," species-quartet(s) selected by random!\n";
			
			$answer = 'r' ; # in case request has been set to 1
			
			my	$n_A	=	@{$href_hol_taxa_of_clan->{$clans[0]}}	;
			my	$n_B	=	@{$href_hol_taxa_of_clan->{$clans[1]}}	;
			my	$n_C	=	@{$href_hol_taxa_of_clan->{$clans[2]}}	;
			my	$n_D	=	@{$href_hol_taxa_of_clan->{$clans[3]}}	;
			
			# Building single quartet numbers
			my @numbers = (0 .. $nq_total-1);
			
			my	%drawn ;
			while ( $quartet_counter < $href_val_of_scr_par->{qlimit}{qnumber} && @numbers >= 1 ){
				
				# count quartet analysis
				$quartet_counter_print++ ;
				##
				
				# Draw a random number and remove it from the array @numbers
				my $index 		= int(rand(scalar( @numbers )));  # Get a random index from 0 to $#numbers
				my $r_number	= splice( @numbers, $index, 1 );  # Remove and return the element at that index
				
				# If this quartet index has already been drawn, skip.
				if ( $drawn{$r_number} ){ die "\nBUG-ERROR: Random number drawn twice in &generate_species_quartets!\nPlease, report BUG to software developer!\n\n" }
				
				# Mark this index as drawn.
				$drawn{$r_number} = 1;
				
				# Calculate the species indices using the mixed-radix approach.
				my $block_size_A	= $n_B * $n_C * $n_D				;
				my $i 				= int($r_number / $block_size_A)	;
				my $R1				= $r_number % $block_size_A			;
				
				my $block_size_B	= $n_C * $n_D				;
				my $j 				= int($R1 / $block_size_B)	;
				my $R2 				= $R1 % $block_size_B		;
				
				my $k 				= int($R2 / $n_D)			;
				my $l 				= $R2 % $n_D				;
				
				
				my $r_quartet		= join ":", ( sort ( $clantaxa1[$i], $clantaxa2[$j], $clantaxa3[$k], $clantaxa4[$l] ));
				#print "\n\trandom quartet of seed number\t", $r_number, ":\t", $r_quartet;
				unless ( $r_quartet =~ /\w+:\w+:\w+:\w+/i ){ die "\nBUG-ERROR: Cannot assign species correctly for quartet ", $r_number, " ('", $r_quartet ,"') in subroutine &generate_species_quartets!\nPlease, report BUG to software developer!\n\n" }
				
				
				
				# check number of sealion informative sites
				&check_site_states(
					
					\$r_quartet									,	# 'tax1:tax2:tax3:tax4'											IN: defined;   OUT. unchanged
					\$quartet_counter							,	# number of quartets accepted for further analysis				IN: defined;   OUT: changed
					\$href_data_of_fileprop->{length}			,	# N sites total													IN: defined;   OUT. unchanged
					\%$href_hol_states_of_t						,	# key1: taxon name; key2: state position number; value: state 	IN: defined;   OUT: unchanged
					\$href_val_of_scr_par->{qlimit}{qlength}	,	# minimum number of acceptable character-only sites				IN: defined;   OUT: unchanged
					\%seen_rejected_quartets					,	# key1: tax1:tax2:tax3:tax4 ; value: 1 							IN: undefined; OUT: defined
					\@$aref_taxon_quartets						,	# list of accepted 4seq combinations							IN: undefined; OUT: defined
					\$$sref_clan_combination					,	# 4-clan string, separated by doublepoints (e.g.: c1:c2:c3:c4)	IN: defined;   OUT: unchanged
					\$href_data_of_fileprop->{type}				,	# sequence type 'aa' or 'nuc'									IN: defined;   OUT: unchanged
				);
				##
				
				if ( $quartet_counter_print % 500 == 0 )							 { print "\n\tN species-quartet(s) build for clade quartet\t"	, $$sref_clan_combination, ":\t", $quartet_counter_print }
				if ( ( $quartet_counter 	% 500 == 0 ) && ( $quartet_counter > 0 )){ print "\n\tN species-quartet(s) selected for clade-quartet\t", $$sref_clan_combination, ":\t", $quartet_counter 		 }
			}
			#######################
		}
		#######################
		...
		#######################
		# proceed with original Nq
		else	{ print "\n\tProceed with all quartets!\n" }
	}
	
	##############################################
	# do all possible species-quartet combinations of actual clade-quartet
	unless ( $answer =~ /r/i ){
			...
			for my $taxon2 ( @clantaxa2 ){
				...
				for my $taxon3 ( @clantaxa3 ){
					...
					for my $taxon4 ( @clantaxa4 ){
						...
						my	 $quartet  =	join ":", ( sort ( $taxon1, $taxon2, $taxon3, $taxon4 ) ); 
						
						# check number of sealion informative sites
						&check_site_states(
							
							\$quartet									,	# 'tax1:tax2:tax3:tax4'											IN: defined;   OUT. unchanged
							\$quartet_counter							,	# number of quartets accepted for further analysis				IN: defined;   OUT: changed
							\$href_data_of_fileprop->{length}			,	# N sites total													IN: defined;   OUT. unchanged
							\%$href_hol_states_of_t						,	# key1: taxon name; key2: state position number; value: state 	IN: defined;   OUT: unchanged
							\$href_val_of_scr_par->{qlimit}{qlength}	,	# minimum number of acceptable character-only sites				IN: defined;   OUT: unchanged
	...
	# print info of accepted quartets
	print "\n\n\tN species-quartet(s) with N fully resolved sites > defined minimum (".$href_val_of_scr_par->{qlimit}{qlength}." sites) for clade-quartet ", $$sref_clan_combination, ": ", $quartet_counter, "\n" ;
	##
	
	##############################################
	# die if no quartet >= minimum of allowed fully resolved sites
	unless ( @$aref_taxon_quartets ){ 
		
		die "\nPARAMETER-ERROR: No species-quartet left with N fully resolved sites > defined minimum (".$href_val_of_scr_par->{qlimit}{qlength}.") for clade-quartet ".$$sref_clan_combination."!\n\n" 
	}
	##############################################
	#exit;
	######################################################################
...
sub check_site_states{
	
	my $sref_taxquartet 			= $_[0];	# 'tax1:tax2:tax3:tax4'																		IN: defined; OUT. unchanged
	my $sref_quartet_counter		= $_[1];	# number of quartets accepted for further analysis											IN: defined; OUT: changed
	my $sref_sites_total			= $_[2];	# N sites total																				IN: defined; OUT. unchanged
	my $href_hol_states_of_tax		= $_[3];	# key1: taxon name; key2: state position number; value: state 								IN: defined; OUT: unchanged
	my $sref_min_sites_allowed		= $_[4];	# minimum number of acceptable character-only sites											IN: defined; OUT: unchanged
	my $href_seen_rejected_quartets	= $_[5];	# key1: tax1:tax2:tax3:tax4 ; value: 1 														IN: undefined; OUT: defined
	my $aref_quartets_accepted		= $_[6]; 	# list of accepted 4seq combinations														IN: undefined; OUT: defined
	my $sref_clade_combination		= $_[7]; 	# 4-clan string, separated by doublepoints (e.g.: c1:c2:c3:c4)								IN: defined; OUT: unchanged
	my $sref_seqtype				= $_[8]; 	# sequence type 'aa' or 'nuc'																IN: defined; OUT: unchanged
	
	######################################################################
	# called from subroutine(s)...
	# &generate_species_quartets
	#
	############
	# using subroutine(s)...
	# ....
	...
	if			( $$sref_seqtype =~ 'nuc'	){ for ( qw/A C G T/									){ $allowed_states{$_}++ } } # all nuc states allowed
	elsif		( $$sref_seqtype =~ 'aa'	){ for ( qw/A C G T N Y R W S K M D V H I E L Q F P/	){ $allowed_states{$_}++ } } # all aa states allowed
	else 		{ die "\nBUG-ERROR: Cannot assign data type in subroutine &perform_quartet_analyses!\nPlease, report BUG to software developer!\n\n" }
	##############################################
	
	##############################################
	# count of character-only site positions defined in %allowed_states
	my	@quartet_taxa	=	split ":", $$sref_taxquartet ;
	my	$counter_inf	=	0 ;
	
	for my $seq_pos ( 0 .. $$sref_sites_total-1 ){ 
						
		# check each site for uninformative site characters and count sealion informative site pattern
		if	( ( $allowed_states{$href_hol_states_of_tax->{$quartet_taxa[0]}[$seq_pos]} ) &&
			  ( $allowed_states{$href_hol_states_of_tax->{$quartet_taxa[1]}[$seq_pos]} ) &&
			  ( $allowed_states{$href_hol_states_of_tax->{$quartet_taxa[2]}[$seq_pos]} ) &&
			  ( $allowed_states{$href_hol_states_of_tax->{$quartet_taxa[3]}[$seq_pos]} ) ){
	...
	##############################################
	# sample quartet for further analysis if number of informative sites satisfyed defined 'qlength' condition
	if ( $counter_inf >= $$sref_min_sites_allowed ){
					
		$$sref_quartet_counter++ ;
		push @$aref_quartets_accepted, $$sref_taxquartet;
	}
			
	else{ $href_seen_rejected_quartets->{$$sref_taxquartet}++ }
	##############################################
}
...
sub latex_quartet_analyses{
	...
	## latex landscape table of rejected species-quartets (filtered|unfiltered)        ##
	#####################################################################################
	if	( 	( $href_print_latex->{table_besttree_frequencies_quartets}			== 1 ) &&
			( $href_value_of_scr_par->{pdf_prtResult}{main} 					== 1 ) ){
		
		for my $ft ( sort keys %{$href_hoh_linedata_of_latexfilename_of_printcode->{latexLQ8}} ){
			
			my	$latex_filename		=	$href_subfolder_q->{tex}."/LQ8_SeaLion_species-quartet_best-tree_frequencies_".$ft.".tex" ;
			print	"\n\t\t...", $latex_filename ;
			
			my	$latex_caption		=	"LQ8 -- Number of clade-quartet related best-tree frequencies in species-quartets." ;
			my	$latex_fileheader	=	shift @{$href_hoh_linedata_of_latexfilename_of_printcode->{latexLQ8}{$ft}} ;
			my	@latex_fileheader	=	split "\t", $latex_fileheader ;
			
			my	@lol_latex_linedata ;
			my	$ycounter	= 0;
			for my $latexline ( @{$href_hoh_linedata_of_latexfilename_of_printcode->{latexLQ8}{$ft}} ){
			...
			my	$ftype	=	$main."_".$ft ;
			push @{$href_hol_pdf_of_type->{$ftype}}, $href_subfolder_q->{tex}."/LQ8_SeaLion_species-quartet_best-tree_frequencies_".$ft.".pdf" ;
		}
	}
	#####################################################################################
	#####################################################################################
...
sub plotsR_quartet_analyses{
	...
	###################################################
	# 18 colors
	my	@colorTable_18a =	(	"#3288bd", "#fee08b"	, "#d53e4f"	, "#99d594", "#fc8d59"	, "#e6f598"	, 
								"#c51b7d", "#fde0ef"	, "#a1d76a"	, "#e9a3c9", "#e6f5d0"	, "#4d9221",
								"#b35806", "#fee0b6"	, "#988ec3"	, "#f1a340", "#d8daeb"	, "#542788"	
							) ;	# MQ3, Q5, T2
	
	my	@colorTable_18b =	(	"#b35806", "#fee0b6"	, "#988ec3"	, "#f1a340", "#d8daeb"	, "#542788",
								"#dc453d", "#fedb7f"	, "#8cbcd9"	, "#fc8d59", "#c0e6f1"	, "#6e9dc7",
								"#c51b7d", "#fde0ef"	, "#a1d76a"	, "#e9a3c9", "#e6f5d0"	, "#4d9221"
							) ;
	
	my	@colorTable_18c =	(	"#c51b7d", "#e9a3c9"	, "#fde0ef"	, "#e6f5d0", "#a1d76a"	, "#4d9221",
								"#b35806", "#fee0b6"	, "#988ec3"	, "#f1a340", "#d8daeb"	, "#542788",
								"#dc453d", "#fedb7f"	, "#8cbcd9"	, "#fc8d59", "#c0e6f1"	, "#6e9dc7"
							) ;	# MQ7
	###################################################
	...
	###################################################
	# 15 colors
	my	@colorTable_15c	=	(	"cornflowerblue", "salmon"	, "mediumseagreen"	, "hotpink1", "gold"	, "gray",	"red",	"cadetblue1",	
								"lightblue4"	, "darkorange"	, "moccasin", "linen"	, "pink",	"violet",	"black"	) ;
	###################################################
	
	###################################################
	# 14 colors
	my	@colorTable_14c	=	(	"cornflowerblue", "salmon"	, "mediumseagreen"	, "hotpink1", "gold"	, "gray",	"cadetblue1"		,
								"lightblue4"	, "darkorange"	, "moccasin", "linen"	, "pink",	"violet"	) ;
	###################################################
	...
	my	@colorTable_3cc	=	(	"#4575b4", "#d73027", "fee090"				) ; # Q1, Q3
	...
	if	( 	( $href_print_Rplot_quartets->{pbarplot_treesuccess_mclan}			== 1 ) &&
		...
		for my	$tsv_mclan_metafileMQ3	( keys %{$href_hoh_tsvdata_of_tsvfilename_of_printcode->{metafileMQ3}} ){
			...
			&r_barplot(	
				...
				\$titl										,	# table title
				\@colorTable_18a							,	# list of specified fill colors, or if undefined using a grey-scaling
	...
	##################################################################################### MQ7 added 15.04.25
	## R barchart-plot of Penguin individually filtered N quartets                     ## works ## main userversion
	#####################################################################################
	if	(	( $href_print_Rplot_quartets->{pbarplot_tree_difference_mclan}		== 1 ) &&
			( $href_value_of_scr_par->{pdf_prtResult}{main} 					== 1 ) ){
		
		for my $tsv_4clan_metafileMQ7 ( sort keys %{$hoh_tsvdata_of_tsvfilename_of_printcode{metafileMQ7}} ){
			
			my	@prts		= split "::", $hoh_tsvdata_of_tsvfilename_of_printcode{metafileMQ7}{$tsv_4clan_metafileMQ7}	; # prts[0]: cladefile; prts[1]: method (nap, napr, or napv); 
			my	$titl		= "Plot MQ7 -- Support distances and rank of method related best-three trees and optionally defined trees to best rooted-clade tree (".$prts[1].")" ;
			
			&r_barplot(	
			
				\$tsv_4clan_metafileMQ7						,	# name of infile table (*.tsv)
				\"Method"									,	# table header of x-plot categories
				\"ScoreDist_to_BestTree"					,	# table header of y-plot values
				\"Tree"										,	# table header (factor) for value separation given different fill colors
				\"Tree"										,	# table header (factor) for barplot frames given different or a single line color(s)
				\"facet"									,	# if defined, facet grid is activated in respect of the next two group parameters
				\"Tree"										,	# table header plot-splitting vertically (defined as dot "." if splitting not needed)
				\"Average"									,	# table header plot-splitting horizontally (undefined "" if splitting not needed)
				\"fixed"									,	# default (undef) all the panels have the same scales ('fixed') can be made independent if defined 'free', 'free_x', or 'free_y'
				\0.4											,	# highlight horizontal line at axis (multiple lines comma separated) interception; neglected if undef
				\"dashed"									,	# horizontal linetype: solid, longdash, dashed, blank, dotdash, dotted, twodash, default if undef
				\1											,	# horizontal linesize; default if undef
				\"gray49"									,	# horizontal line color, default if undef
				\0											,	# 0 -> a stacked barplot is created by default, 1 -> non-stagged barplot with each value of the same x-plot category in a own x-axis position
				\1											,	# 0 -> no labels at barchart, 1 -> label (geom_text) at barchart
				\"Rank"										,	# table header of barchart label
				\"-0.1"										,	# vertical shift value (integer, float, positive or negative) of barchart label
				\"black"									,	# barchart label color
				\5											,	# barchart label size 
				\$titl										,	# table title
				\@colorTable_18c							,	# list of specified fill colors, or if undefined using a grey-scaling
				\@colorTable_1c								,	# list of specified line color(s)
				\"Filter Method"							,	# label x-axis
				\1											,	# if value == 1 -> rotate x-axis labels via 90°, undef (0) keeps labels horizontally
				\"Score Distance to Best Tree"				,	# labe y-axis
				\"Rooted Clade Tree"						,	# box colour label
				\40											,	# width of pdf graphic, keep undef for def width: 40
				\15											,	# height of pdf graphic, keep undef for def height: 15
				\"barchart"									,	# pdf file suffix to avoid different plots of identic names
				\$href_subfolder_q->{svg}					,	# svg print folder
	...
	if	( 	( $href_print_Rplot_quartets->{pline_taxreject_clan4}				== 1 ) &&
		...
		for my $tsv_4clan_metafileQ5 ( sort keys %{$href_hoh_tsvdata_of_tsvfilename_of_printcode->{metafileQ5}} ){
			...
			&r_lineplot(	
				...
				\"gray49"									,	# vertical line color, default if undef
				\$titl										,	# table title
				\@colorTable_18a							,	# list of specified line colors
	...
	##################################################################################### Q8 added 15.04.25
	## R heatmap-plot of clade-quartet individually filtered % quartets                ## works ## main
	#####################################################################################
	if	(	( $href_print_Rplot_quartets->{pheatmap_filtered_Pquartets}			== 1 ) &&
			( $href_value_of_scr_par->{pdf_prtResult}{main} 					== 1 ) ){
	
		for my $tsv_4clan_metafileQ8 ( sort keys %{$hoh_tsvdata_of_tsvfilename_of_printcode{metafileQ8}} ){
			
			my	$ctype		= $hoh_tsvdata_of_tsvfilename_of_printcode{metafileQ8}{$tsv_4clan_metafileQ8}	;
			my	$title		= "Plot Q8 -- Percentage of rejected species contributions in single clade-quartets (".$ctype.")" ;
			my	$ftype	=	$main."_".$ctype ;
			
			&r_heatmap(
				
				\$tsv_4clan_metafileQ8						,	# name of infile table (*.tsv)
				\"heatmap"									,	# pdf file suffix to avoid different plots of identic names
				\$href_subfolder_q->{svg}					,	# subfolder of svg print
				\$href_subsubfolder_q->{pdf_single}			,	# subfolder of pdf print
				\@$aref_pdf_pattern							,	# list of pdf prints, subsequently collected via pdfunite
				\40											,	# pdf height
				\40											,	# pdf width
				\"Filter"									,	# x-value:			table header of x-plot categories
				\"Species"									,	# y-value:			table header of y-plot categories
				\"P_Rejected_Quartets"						,	# box-value:		parameter separation given different fill colors
				\"facet"									,	# facet-activation:	if defined, facet grid is activated in respect of the next two group parameters
				\"Clade_Quartet"							,	# facet-vertical: 	table header plot-splitting vertically (defined as dot "." if splitting not needed)
				#\"Clade"									,	# facet-horizontal	table header plot-splitting horizontally (undefined "" if splitting not needed)
				\""											,	# facet-horizontal	table header plot-splitting horizontally (undefined "" if splitting not needed)
				\""											,	# facet-scaling:	default (undef) all the panels have the same scales ('fixed') can be made independent if defined 'free', 'free_x', or 'free_y'
				\"Spectral"									,	# boxcolor-sheme:	specified fill color
				\"90"										,	# rotate-xlabel:	rotate x-axis labels via defined degree (e.g. 90 -> 90°, undef (0) keeps labels horizontally
				\0											,	# fixedsquares:		if value == 1 -> heatmap with fixed squares, independent of figure space (coord_fixed option)
				\1											,	# boxlineactive:	if value == 1 -> geomtext activated, printing values to boxes
				\"P_Rejected_Quartets"						,	# boxvalue: 		tsv header of print values for each box (needed if geomtext is activated)
				\"black"									,	# boxtextcolor: 	color of print values for each box (color = "black" if undef)
				\4											,	# boxtextsize: 		size of print values for each box (size = "4" if undef)
				\1											,	# boxlineactive: 	if value == 1 -> boxlines activated (with default or further specified setup)
				\"white"									,	# boxlinecolor: 	color of each box line ("white" if undef)
				\1.5										,	# boxlinesize: 		witdh of each box line (size = "1.5" if undef)
				\0											,	# scalefilln: 		number of colors passing the defined color palette; "scale_fill_distiller" used instead of "scale_fill_gradientn" if undef
				\$title										,	# title: 			grafic title line
				\1											, 	# legendactive: 	if value == 1 -> activation of legend customization; if value == 2 -> no legend
				\2											,	# legendwidth:		legend width, only active if $legendactive == 1
				\10											,	# legendheight:		legend height, only active if $legendactive == 1
				\""											,	# legendtitle:		legent title, independent of $legendactive, table header used if undef
			) ;
	...
	## R heatmap-plot of clade-quartet individually filtered % quartets                ## works ## main
	#####################################################################################
	if	(	( $href_print_Rplot_quartets->{pheatmap_singlespecies_support}		== 1 ) &&
			( $href_value_of_scr_par->{pdf_prtResult}{main} 					== 1 ) ){
	
		for my $tsv_4clan_metafileT9 ( sort keys %{$hoh_tsvdata_of_tsvfilename_of_printcode{metafileT9}} ){
			
			my	@prts	= split ":", $hoh_tsvdata_of_tsvfilename_of_printcode{metafileT9}{$tsv_4clan_metafileT9}	;
			my	$ctype	= $prts[0] ;
			my	$suppo	= $prts[1] ;
			my	$title	= "Plot T9 -- Species ".$suppo." support contribution in single clade-quartets (".$ctype.")" ;
			my	$ftype	=	$main."_".$ctype ;
			
			&r_heatmap(
				
				\$tsv_4clan_metafileT9						,	# name of infile table (*.tsv)
				\"heatmap"									,	# pdf file suffix to avoid different plots of identic names
				\$href_subfolder_q->{svg}					,	# subfolder of svg print
				\$href_subsubfolder_q->{pdf_single}			,	# subfolder of pdf print
				\@$aref_pdf_pattern							,	# list of pdf prints, subsequently collected via pdfunite
				\40											,	# pdf height
				\40											,	# pdf width
				\"Tree"										,	# x-value:			table header of x-plot categories
				\"Species"									,	# y-value:			table header of y-plot categories
				\"Support"									,	# box-value:		parameter separation given different fill colors
				\"facet"									,	# facet-activation:	if defined, facet grid is activated in respect of the next two group parameters
				\"Clade_Quartet"							,	# facet-vertical: 	table header plot-splitting vertically (defined as dot "." if splitting not needed)
				\"Filter"									,	# facet-horizontal	table header plot-splitting horizontally (undefined "" if splitting not needed)
				\""											,	# facet-scaling:	default (undef) all the panels have the same scales ('fixed') can be made independent if defined 'free', 'free_x', or 'free_y'
				\"Spectral"									,	# boxcolor-sheme:	specified fill color
				\"90"										,	# rotate-xlabel:	rotate x-axis labels via defined degree (e.g. 90 -> 90°, undef (0) keeps labels horizontally
				\0											,	# fixedsquares:		if value == 1 -> heatmap with fixed squares, independent of figure space (coord_fixed option)
				\1											,	# boxlineactive:	if value == 1 -> geomtext activated, printing values to boxes
				\"Support"									,	# boxvalue: 		tsv header of print values for each box (needed if geomtext is activated)
				\"black"									,	# boxtextcolor: 	color of print values for each box (color = "black" if undef)
				\4											,	# boxtextsize: 		size of print values for each box (size = "4" if undef)
				\1											,	# boxlineactive: 	if value == 1 -> boxlines activated (with default or further specified setup)
				\"white"									,	# boxlinecolor: 	color of each box line ("white" if undef)
...
sub print_tsv_MQ7{
	...
	my	$href_printpath						= $_[0] ; # key1: R grafic unique printcode (e.g. 'metafileQ1'); key2: *.tsv filename; value: *tsv filename data addings 			IN: defined; OUT: unchanged
	my	$href_hoh_data_of_tsvfile_of_prc	= $_[1] ; # key: output-type (e.g. 'pdf'); value: pathdir																			IN: defined; OUT: changed
	my	$href_rctrees_of_ts					= $_[2] ; # key1: average (mean|median); key2: newick trees; value: counter best three trees										IN: defined; OUT: unchanged
	my	$href_s_of_t						= $_[3] ; # key1: mean|median; key2: tree; value: support																			IN: defined; OUT: unchanged
	my	$href_p_of_t						= $_[4] ; # key1: mean|median; key2: tree; value: support ranking																	IN: defined; OUT: unchanged
	my	$sref_recoded_fm					= $_[5] ; # method code for print out																								IN: defined; OUT: unchanged
	my	$sref_cm							= $_[6] ; # actual method code (nap, napr, napv)																					IN: defined; OUT: unchanged
	my	$sref_ts							= $_[7] ; # support type: mean|median																								IN: defined; OUT: unchanged
	my	$sref_s_besttree					= $_[8] ; # support of best file tree																								IN: defined; OUT: unchanged
	my	$sref_clfile						= $_[9] ; # name of given clanfile																									IN: defined; OUT: unchanged
	...
	######################################################################
	## print rooted-clade tree support of different methods to tsv table MQ7
	######################################################################
	
	####################################
	# open filehandle
	my	 	$tsv_mclan_metafileMQ7 	= $href_printpath->{tsv}."/MQ7_SeaLion_metaData_rooted-clade_tree_support_info_".$$sref_cm.".tsv" ;
	open	OUTtsvMQ7, ">>$tsv_mclan_metafileMQ7" || die "Cannot write tsv file ", $tsv_mclan_metafileMQ7, ": $!" 	;
	####################################
	
	####################################
	# print header only once to each clanfile corresponding tsv table file
	unless ( $href_hoh_data_of_tsvfile_of_prc->{metafileMQ7}{$tsv_mclan_metafileMQ7} ){
		
		########
		# defined header line of tsv outfile
		print OUTtsvMQ7	"Method"					,
						"\tAverage"					,
						"\tTree"					,	
						"\tScore"					,
						"\tScoreDist_to_BestTree"	,
						"\tRank"					,
						"\n"						;
		########
		...
		########
		# count metafile header +1
		$href_hoh_data_of_tsvfile_of_prc->{metafileMQ7}{$tsv_mclan_metafileMQ7} = $$sref_clfile."::".$$sref_cm ;
		########
	}
	####################################
	...
	####################################
	# print table data
	for my $tree ( sort keys %{$href_rctrees_of_ts->{$$sref_ts}}){
		
		#my $sdist_to_best = sprintf ( "%.2f", ( $href_s_of_t->{$tree} - $$sref_s_besttree ) );
		my $sdist_to_best = sprintf ( "%.2f", ( $$sref_s_besttree - $href_s_of_t->{$tree} ) );
		
		print OUTtsvMQ7		 $$sref_cm."_".$$sref_recoded_fm	,
						"\t".$$sref_ts							,
						"\t".$tree								,
						"\t".$href_s_of_t->{$tree}				,
						"\t".$sdist_to_best						,
...
sub print_tsv_MQ9_LQ8{ 
	...
	my	$href_hoh_reje_of_4clan_of_ft_of_fn						=	$_[4]; # key1: 4clan code; key2: type of supportfilter ('nap_risk1', 'napr_risk1', or 'napv_risk1'); key3: quartet number; value: score-value	IN: defined; OUT: unchanged 
	my	$href_algorithm_def_methods								=	$_[5]; # key1: methode type; key2: method code; value: integer																					IN: defined; OUT: unchanged
	my	$href_seen_filter_method								=	$_[6]; # key1: filter method incl OTU clipping approaches; value: seen +1																		IN: defined; OUT: unchanged
	my	$href_clan4_of_tree										=	$_[7]; # key1: 4clan tree; value: 4clan code																									IN: defined; OUT: unchanged
	my	$href_textfilename_of_prtcode							=	$_[8]; # key1: textfile unique printcode (e.g. 'textA'); value: filename for print																IN: defined; OUT: changed
	my	$href_hoh_linedata_of_latexfile_of_prtcode				=	$_[9]; # key1: Latex table unique printcode (e.g. 'latexC'); key2: *.tex filename; value: @list of assigned table lines							IN: defined; OUT: changed
	...
	############
	# using subroutine(s)...
	# &quartet_besttree_frequency_test
	######################################################################
	
	######################################################################
	## Count+Print best-tree frequencies and calculate stats
	######################################################################
	for my $cm_org ( sort keys %{$href_algorithm_def_methods->{original}} ){ #print "\ncmorg: ", $cm_org;
		
		######################################################################
		## Count total number of species-quartet best tree support for each clade-tree
		######################################################################
		
		my %seen_best_tree ;
		my %seen_filter_types ;
		for my $qn ( sort keys %{$href_bestTree_of_fc_of_cm} ){
			
			my	@prts 		= split ":", $href_bestTree_of_fc_of_cm->{$qn}{$cm_org} ;
			my	$besttree	= $prts[0] ;
			
			$seen_best_tree{$besttree}{unfiltered}++ ;
			$seen_filter_types{unfiltered}++ ;
			#print "\nqn: ", $qn, "\tbest tree:\t", $besttree, "\tseen unfiltered:", $seen_best_tree{$besttree}{unfiltered} ;
			...
			for my $fm ( sort keys %{$href_seen_filter_method} ){
				...
				if ( $fm =~ /^${cm_org}_/ ){ 
					...
					my	 $qfilter_type	=	&recode_filter_printout( \$fm )	;
					$seen_filter_types{$qfilter_type}++ ;
					
					unless ( $href_hoh_reje_of_4clan_of_ft_of_fn->{$$sref_clan4}{$fm}{$qn} ){
						
						$seen_best_tree{$besttree}{$qfilter_type}++ ;
						#print "\nseen ", $qfilter_type, ":\t", $besttree, "\t", $seen_best_tree{$besttree}{$qfilter_type};
					}
				}
			}
		}
		...
		######################################################################
		## print header and data to tsv MQ9 and txt LQ8, sample data tex LQ8 
		######################################################################
		
		########
		# open tsv
		my	 	$tsv_4clan_metafileMQ9 = $href_printpath->{tsv}."/MQ9_SeaLion_metaData_species-quartet_N_best-tree-support_".$cm_org.".tsv" ;
		open 	OUTtsvMQ9, ">>$tsv_4clan_metafileMQ9" || die "Cannot write tsv file ", $tsv_4clan_metafileMQ9, ": $!" ;
		########
		
		########
		# do header
		my @filter_types	= sort keys %seen_filter_types ;
		my $filter_header	= join "\t", @filter_types ;
		my $header 			= "4Clade"."\t"."Tree"."\t".$filter_header ;
		########
		
		########
		# sample latex header as first list element
		push @{$href_hoh_linedata_of_latexfile_of_prtcode->{latexLQ8}{$cm_org}}, $header ;	
		########
		
		########
		# print header tsv
		unless ( $href_hoh_data_of_tsvfile_of_prcode->{metafileMQ9}{$tsv_4clan_metafileMQ9} ){ 
			
			print OUTtsvMQ9 $header."\n" ;
			$href_hoh_data_of_tsvfile_of_prcode->{metafileMQ9}{$tsv_4clan_metafileMQ9} = $cm_org ;
		}
		########
		
		########
		# open txt
		my		$f_pinfo = $href_printpath->{txt}."/".$href_textfilename_of_prtcode->{txtLQ8} ;
		open 	OUTtxt,	">>$f_pinfo" or die "\nTEXTFILE-ERROR: Cannot write to ", $f_pinfo, "!\n\n" ;
		########
		
		######## txt
		# print header only once
		unless ( $href_textfilename_of_prtcode->{txtLQ8hcode} ){ 
			
			print OUTtxt $header."\tScore\n" ;
			$href_textfilename_of_prtcode->{txtLQ8hcode}++
		}
		########
		
		########
		# print data
		my %seen_filter_with_data ;
		for my $tr ( sort keys %{$href_clan4_of_tree} ){
			
			my $l_data = $$sref_clan4."\t".$tr ;
			
			for my $filter ( @filter_types ){
				
				unless ( $seen_best_tree{$tr}{$filter} ){ 
					
					$l_data .= "\t"."0" ;
					$seen_best_tree{$tr}{$filter} = 0 ;				
				}
				else{ 
					
					$l_data .= "\t".$seen_best_tree{$tr}{$filter} ;
					$seen_filter_with_data{$filter}++ ;
				}
			}
			
			print OUTtsvMQ9 $l_data."\n";
			print OUTtxt 	$l_data."\t".$cm_org."\n";
			
			push @{$href_hoh_linedata_of_latexfile_of_prtcode->{latexLQ8}{$cm_org}}, $l_data ;
		}
		########
		...
		######################################################################
		# G-test, binomial, coalescent branch length calculations
		######################################################################
		for my $ft ( sort keys %seen_filter_with_data ){
			
			my @trees 	= sort keys %{$href_clan4_of_tree} ;
			my @n_best	= ( $seen_best_tree{$trees[0]}{$ft}, $seen_best_tree{$trees[1]}{$ft}, $seen_best_tree{$trees[2]}{$ft} ) ;
			
			&quartet_besttree_frequency_test(
				
				\@trees							, # sorted set of the three possible clade-quartet trees																IN: defined; OUT: unchanged
				\@n_best						, # number of best-tree support in order of @trees																		IN: defined; OUT: unchanged
				\%$href_printpath				, # key1: R grafic unique printcode (e.g. 'metafileQ1'); key2: *.tsv filename; value: *tsv filename data addings 		IN: defined; OUT: unchanged
				\%$href_textfilename_of_prtcode	, # key1: textfile unique printcode (e.g. 'textA'); value: filename for print											IN: defined; OUT: changed
				\$cm_org						, # split-calculation method (nap, napr, or napv)																		IN: defined; OUT: unchanged
				\$$sref_clan4					, # clan4 combination of given quartet combination																		IN: defined; OUT: unchanged
				\$ft							, # filter type																											IN: defined; OUT: unchanged
			) ;
		}
		######################################################################
	}
...
sub print_tsv_Q5_Q8{
	...
	for	my $ftype ( sort keys %$href_seen_filt_method ){ 
		...
		my	 	$tsv_4clan_metafileQ8	=	$href_printpath->{tsv}."/Q8_SeaLion_metaData_clade-quartet_species-specific_quartet-particpations_all_filter_".$cm_fm[0].".tsv" ;
		open 	OUTtsvQ8, ">>$tsv_4clan_metafileQ8" || die "Cannot write tsv file ", $tsv_4clan_metafileQ8, ": $!" ;
		########
		...
		########
		# defined header line of tsv outfile Q5
		print 	OUTtsvQ5	"Species"						,
							...
							"\tN_Quartets_of_Clade_Quartet"	,
							"\n"							;
		########
		
		########
		# defined header line of tsv outfile Q5
		unless ( $seen_headerQ8{$cm_fm[0]} ){
			
			print 	OUTtsvQ8	"Species"						,
								"\tClade"						,
								"\tScore"						,
								"\tFilter"						,
								"\tClade_Quartet"				,
		...
		$href_hoh_data_of_tsvfile_of_prcode->{metafileQ8}{$tsv_4clan_metafileQ8} = $cm_fm[0] ;
		########
		...
		for my $t ( sort keys %$href_clan_of_seq ){
			...
			for my $clan4 ( sort keys %$href_seen_clan4 ){
				...
				############
				# percent species related quartets of 4clan
				my $total_n  = $n_rej_qn + $n_rem_qn ;
				my $p_rem_qn = 'NA' ;
				my $p_rej_qn = 'NA' ;
				if ( $total_n ){ 
					
					$p_rem_qn = &calc_percent( $n_rem_qn, $total_n ) ;
					$p_rej_qn = &calc_percent( $n_rej_qn, $total_n )
				}
				############
				...
				print	OUTtsvQ5	$t								,	# OTU
									...
									"\t",$total_n					,	# N species quartet contrbutions of single 4clade combination
									"\n"							;
				
				print	OUTtsvQ8	$t								,	# OTU
									"\t",$href_clan_of_seq->{$t}	,	# Clan
									"\t",$cm_fm[0]					,	# Score
									"\t",$filter					,	# Filter
									"\t",$clan4						,	# Clan4
									"\t",$p_rem_qn					,	# remain4clan percentage
									"\t",$p_rej_qn					,	# rejected4clan percentage
									"\t",$total_n					,	# N species quartet contrbutions of single 4clade combination
									"\n"							;
			}
			########
...
sub print_tsv_T9{ 
	...
	for my $taxon ( sort keys %{$href_seen_alltax_of_4clan->{$$sref_clan4}} ){
		...
		for my $stype ( @{$href_alg_def_methods->{qsuppo}} ){
			...
			my	 	$tsv_4clan_metafileT9 = $href_prtpath->{tsv}."/T9_SeaLion_metaData_clade-quartet_species-specific_support_contributions_".$stype."_".$cm_fm[0].".tsv" ;
			open 	OUTtsvT9, ">>$tsv_4clan_metafileT9" || die "Can't write tsv file ", $tsv_4clan_metafileT9, ": $!" ;
			########
			
			########
			# print header only once to each clanfile corresponding tsv table file
			unless ( $href_hoh_data_of_tsvfile_of_prtcode->{metafileT9}{$tsv_4clan_metafileT9} ){
				
				#print "::printing table header to ", $tsv_4clan_metafileT2 ,"\n" ;
				
				########
				# defined header line of tsv outfile
				print	OUTtsvT9	"Score\t", "Filter\t", "Species\t", "Clade\t", "Clade_Quartet\t", "Average\t", "Tree_Code\t", "Tree\t", "Support";
				########
				
				########
				# count metafile header +1
				$href_hoh_data_of_tsvfile_of_prtcode->{metafileT9}{$tsv_4clan_metafileT9} = $cm_fm[0].":".$stype ;
				########
			}
			########
			...
			if ( $href_supp_of_4clan_of_tax_of_type_of_tree->{$$sref_clan4}{$taxon} ){ #print "\ntaxon ", $taxon, "\t4clan ", $$sref_clan4, "\n";
				...
				for my $tree ( sort keys %{$href_supp_of_4clan_of_tax_of_type_of_tree->{$$sref_clan4}{$taxon}{$stype}} ){
					...
					my $support = sprintf ( "%.2f", ( $href_supp_of_4clan_of_tax_of_type_of_tree->{$$sref_clan4}{$taxon}{$stype}{$tree} ));
...
sub quartet_besttree_frequency_test{ 
	
	my $aref_trees				= $_[0] ;	# sorted set of the three possible clade-quartet trees																IN: defined; OUT: unchanged
	my $aref_n_best				= $_[1] ;	# sorted set of the three possible clade-quartet trees																IN: defined; OUT: unchanged
	my $href_prtpath			= $_[2] ;	# key1: R grafic unique printcode (e.g. 'metafileQ1'); key2: *.tsv filename; value: *tsv filename data addings 		IN: defined; OUT: unchanged
	my $href_textfname_of_prt	= $_[3] ;	# key1: textfile unique printcode (e.g. 'textA'); value: filename for print											IN: defined; OUT: changed
	my $sref_cm_org				= $_[4] ;	# split-calculation method (nap, napr, or napv)																		IN: defined; OUT: unchanged
	my $sref_cq					= $_[5] ;	# clan4 combination of given quartet combination																	IN: defined; OUT: unchanged
	my $sref_ft					= $_[6] ;	# filter type																										IN: defined; OUT: unchanged
	...
	$ab //= $aref_n_best->[0]; $ac //= $aref_n_best->[1]; $bc //= $aref_n_best->[2];  

	my $n = $ab + $ac + $bc;
	die "Total n must be > 0\n" if $n <= 0;
	
	# Pack with labels
	my @cats = (
	
		{ label	=>	$aref_trees->[0], n=>$ab },
		{ label	=>	$aref_trees->[1], n=>$ac },
		{ label	=>	$aref_trees->[2], n=>$bc },
	);
	
	# Sort by count desc to find concordant & discordants
	@cats = sort { $b->{n} <=> $a->{n} } @cats;
	my $concordant		=  $cats[0];
	my ($disc1, $disc2) = ($cats[1], $cats[2]);
	
	# Tie check for the top two
	my $tie_top = ($cats[0]{n} == $cats[1]{n}) ? 1 : 0;
	########################################################
	
	
	########################################################
	# -------- (a) Multinomial G-test vs polytomy (1/3,1/3,1/3) --------
	my $Ei		= $n / 3.0;
	my $G  		= term_g($ab, $Ei) + term_g($ac, $Ei) + term_g($bc, $Ei);
	my $p_chi2	= exp(-$G/2.0);  # df=2 tail
	
	# Proportions + Wilson 95% CIs
	my $z = 1.959963984540054;
	my ($p_ab, $lo_ab, $hi_ab) = wilson_ci($ab, $n, $z);
	my ($p_ac, $lo_ac, $hi_ac) = wilson_ci($ac, $n, $z);
	my ($p_bc, $lo_bc, $hi_bc) = wilson_ci($bc, $n, $z);
	
	# map prop/CIs by label for convenience
	my %prop = (
	
		$aref_trees->[0] => [$p_ab,$lo_ab,$hi_ab],
		$aref_trees->[1] => [$p_ac,$lo_ac,$hi_ac],
		$aref_trees->[2] => [$p_bc,$lo_bc,$hi_bc],
	);
	########################################################
	
	
	########################################################
	# -------- (b) Discordant symmetry (binomial) --------
	my $Ndisc = $disc1->{n} + $disc2->{n};
	my ($disc_larger, $disc_smaller) = ($disc1->{n} >= $disc2->{n}) ? ($disc1, $disc2) : ($disc2, $disc1);
	
	my ($binom_p, $p_disc_prop, $lo_disc_prop, $hi_disc_prop);
	if (!$tie_top && $Ndisc > 0) {
		
		# two-sided exact p = 2 * upper tail at larger of the two discordants
		my $k_hi 		= $disc_larger->{n}							;
		my $log_upper 	= log_binom_tail_upper($Ndisc, 0.5, $k_hi)	;
		my $upper 		= exp($log_upper)							;
		$binom_p 		= 2.0 * $upper								;
		$binom_p 		= 1.0 if $binom_p > 1.0						;
		
		# Proportion of disc_larger among discordants, with Wilson CI
		($p_disc_prop, $lo_disc_prop, $hi_disc_prop) = wilson_ci($disc_larger->{n}, $Ndisc, $z);
	} 
	else {
	...
	########################################################
	# -------- (c) Coalescent branch length t and 95% CI --------
	# Choose the dominant topology as concordant (largest of ab, ac, bc)
	my $pc    			= $concordant->{n} / $n;
	my ($pc_lo, $pc_hi) = @{$prop{$concordant->{label}}}[1,2];
	
	my ($that, $t_lo, $t_hi, $t_note) = ('NA','NA','NA','');
	if (!$tie_top) {
		
		if ($pc >= 1/3)  { $that = -log(1.5 * (1.0 - $pc)); }
		else             { $t_note = "pc < 1/3; t not defined under MSC"; }

		if (defined $pc_lo && $pc_lo >= 1/3) { $t_lo = -log(1.5 * (1.0 - $pc_lo)); }
		if (defined $pc_hi && $pc_hi >= 1/3) { $t_hi = -log(1.5 * (1.0 - $pc_hi)); }
		if ((!defined $t_lo || !defined $t_hi) || ($pc_lo < 1/3 || $pc_hi < 1/3)) {
		
			$t_note = length($t_note) ? $t_note : "CI clipped: pc CI crosses 1/3";
		}
	} 
	else { $t_note = "topology tie: t not estimated" }
	########################################################
	...
	########
	# open txt
	my		$f_pinfo = $href_prtpath->{txt}."/".$href_textfname_of_prt->{txtLQ9} ;
	open 	OUTtxt,	">>$f_pinfo" or die "\nTEXTFILE-ERROR: Cannot write to ", $f_pinfo, "!\n\n" ;
	########
	
	########
	# print general result info
	print  OUTtxt	"-------------------------------------------------\nMethod:\t", $$sref_cm_org, "\nFilter:\t", $$sref_ft, "\nClade-quartet:\t", $$sref_cq, "\n" ;
	print  OUTtxt	"SeaLion quartet counts:\n";
	printf OUTtxt	"\t$aref_trees->[0] = %d, $aref_trees->[1] = %d, $aref_trees->[2] = %d (n = %d)\n\n", $ab, $ac, $bc, $n;
	
	printf OUTtxt	"(a) Multinomial G-test vs polytomy (1/3,1/3,1/3): G = %.4f, df = 2, p = %.3e\n", $G, $p_chi2;
	printf OUTtxt	"\tProportions (95%% Wilson CI):\n";
	printf OUTtxt	"\t$aref_trees->[0]: %.5f [%.5f, %.5f]\n"  , $p_ab, $lo_ab, $hi_ab;
	printf OUTtxt	"\t$aref_trees->[1]: %.5f [%.5f, %.5f]\n"  , $p_ac, $lo_ac, $hi_ac;
	printf OUTtxt	"\t$aref_trees->[2]: %.5f [%.5f, %.5f]\n\n", $p_bc, $lo_bc, $hi_bc;
	
	if ($tie_top) {
		
		print OUTtxt 	"(b) Discordant symmetry: topologies are tied for largest; symmetry test not applicable.\n\n" ;
	} 
	else {
		
		printf OUTtxt	"(b) Discordant symmetry on discordants {%s=%d, %s=%d} (total=%d): exact two-sided p = %s\n",
						$disc1->{label}, $disc1->{n}, $disc2->{label}, $disc2->{n}, $Ndisc, fmt($binom_p)	;
		
		printf OUTtxt	"    Proportion of larger discordant (%s) among discordants: %s [%s, %s]\n\n",
						$disc_larger->{label}, fmt($p_disc_prop), fmt($lo_disc_prop), fmt($hi_disc_prop)	;
	}
	
	printf OUTtxt	"(c) Coalescent branch length t (concordant = %s): t_hat = %s",
					$concordant->{label}, fmt_or_na($that);
	
	if ( defined $t_lo && defined $t_hi && $t_lo ne 'NA' && $t_hi ne 'NA' ) {
		
		printf OUTtxt	"  (95%% CI: %s, %s)", fmt($t_lo), fmt($t_hi) ;
	}
	
	print OUTtxt ($t_note ? "  [$t_note]\n\n" : "\n\n");
	print OUTtxt "CSV data line:\n" ;
	########
	
	########
	# CSV header + row
	print OUTtxt join(",", qw(
	n_ab n_ac n_bc n_total
	p_ab ci_ab_low ci_ab_high
	p_ac ci_ac_low ci_ac_high
	p_bc ci_bc_low ci_bc_high
	G p_chi2
	N_disc k_ac binom_p
	p_ac_disc ci_ac_disc_low ci_ac_disc_high
	concordant p_conc ci_conc_low ci_conc_high
	t_hat t_lo t_hi t_note
	)), "\n";

	print OUTtxt join(",", (
	$ab, $ac, $bc, $n,
	map(fmt($_), ($p_ab,$lo_ab,$hi_ab)),
	map(fmt($_), ($p_ac,$lo_ac,$hi_ac)),
	map(fmt($_), ($p_bc,$lo_bc,$hi_bc)),
	fmt($G), fmt($p_chi2),
	$concordant->{label}, $concordant->{n},
	fmt($pc), fmt($prop{$concordant->{label}}[1]), fmt($prop{$concordant->{label}}[2]),
	$disc1->{label}, $disc1->{n}, $disc2->{label}, $disc2->{n}, $Ndisc,
	fmt($binom_p),
	(defined $binom_p && $binom_p ne 'NA') ? (
    $disc_larger->{label}, fmt($p_disc_prop), fmt($lo_disc_prop), fmt($hi_disc_prop)
	) : ('NA','NA','NA','NA'),
	fmt_or_na($that), fmt_or_na($t_lo), fmt_or_na($t_hi),
	($t_note ? ($t_note =~ s/,/;/gr) : '')
	)), "\n";
	########################################################
	...
	sub wilson_ci {
		...
		my $z2 		= $z*$z;
		my $den 	= 1.0 + $z2/$n_pos;
		my $center 	= $phat + $z2/(2.0*$n_pos);
		my $rad 	= $z * sqrt( ($phat*(1.0-$phat) + $z2/(4.0*$n_pos)) / $n_pos );
		my $lo 		= ($center - $rad) / $den;
		my $hi 		= ($center + $rad) / $den;
		$lo 		= 0.0 if $lo < 0.0;
		$hi 		= 1.0 if $hi > 1.0;
	...
	########
	# exact log upper-tail of Binomial(N, p) at k (sum i=k..N)
	sub log_binom_tail_upper {
		
		my ($N, $p, $k) = @_;
		die "Need 0 <= k <= N" unless defined $N && defined $p && defined $k && $k >= 0 && $k <= $N;
		
		return 0.0    if $k == 0;   # log(1)
		return -1e300 if $k >  $N;  # ~ -Inf
		
		my $log1mp = ($p == 1.0) ? -1e300 : log(1.0 - $p);
		my $logp   = ($p == 0.0) ? -1e300 : log($p);
		my $log_sum;  # undef ~ -Inf
		
		for my $i ($k .. $N) {
			
			my $logC = lgamma($N+1) - lgamma($i+1) - lgamma(($N-$i)+1);
			my $term = $logC + $i*$logp + ($N-$i)*$log1mp;
	...
	########
	sub fmt { my ($x)=@_; return 'NA' unless defined $x; return sprintf("%.6g",$x) }
	########
	
	########
	sub fmt_or_na { my ($x)=@_; return defined $x ? sprintf("%.6g",$x) : 'NA' }
	########################################################
	#exit;
}
...
sub quartet_building_and_p4_main{ 
	...
	for my $clan4 ( sort @$aref_clanquartets ){
		...
		my	@taxon_quartets	= ()	;	# list of 4tax quartets with remaining sequence length after gap+amb exclusions >= defined length minimum 
		&generate_species_quartets(
		...
		for my $tquartet ( @taxon_quartets ){
			...
			( my $msa_p4 = $href_value_of_scr_par->{sys_parameter}{msa_nopath} ) =~ s/\.fas$|\.fasta$|\.phy$|\.phylip$//i ;
			...
			(   my $script_p4 = $href_value_of_scr_par->{sys_parameter}{msa_nopath} ) =~ s/\.fas$|\.fasta$|\.phy$|\.phylip$//i ;
				my @qtrees_p4 ; # P4 trees in the same order as defined in P4 input skript
...
sub r_heatmap{ 
	...
	my	$sref_intable		=	$_[0]	;	# name of infile table (*.tsv)
	my	$sref_filesuffix	=	$_[1]	;	# pdf file suffix to avoid different plots of identic names
	my	$sref_svg_folder	=	$_[2]	;	# subfolder of svg print
	my	$sref_pdf_folder	=	$_[3]	;	# subfolder of pdf print
	my	$aref_pdf_sampled	=	$_[4]	;	# list of pdf prints, subsequently collected via pdfunite
	my	$sref_pdf_height	=	$_[5]	;	# height of pdf graphic, keep undef for def height: 15
	my	$sref_pdf_width		=	$_[6]	;	# width of pdf graphic, keep undef for def width: 40
	my	$sref_x_theader		=	$_[7]	;	# table header of x-plot categories
	my	$sref_y_theader		=	$_[8]	;	# table header of y-plot values
	my	$sref_group_fill	=	$_[9]	;	# parameter separation given different fill colors
	my	$sref_flag_facet	=	$_[10]	;	# if defined, facet grid is activated in respect of the next two group parameters
	my	$sref_plot_split_v	=	$_[11]	;	# table header plot-splitting vertically (defined as dot "." if splitting not needed)
	my	$sref_plot_split_h	=	$_[12]	;	# table header plot-splitting horizontally (undefined "" if splitting not needed)
	my	$sref_scale_facet	=	$_[13]	;	# default (undef) all the panels have the same scales ('fixed') can be made independent if defined 'free', 'free_x', or 'free_y'
	my	$sref_fillcolor		=	$_[14]	;	# specified fill color
	my	$sref_rotate_xlabel	=	$_[15]	;	# if value == 1 -> rotate x-axis labels via 90°, otherwise keep labels horizontally
	my	$sref_coord_fix		=	$_[16]	;	# if value == 1 -> heatmap with fixed squares, independent of figure space (coord_fixed option)
	my	$sref_geomtext		=	$_[17]	;	# if value == 1 -> geomtext activated, printing values to boxes
	my	$sref_boxtext		=	$_[18]	;	# tsv header of print values for each box (needed if geomtext is activated)
	my	$sref_boxtextcolor	=	$_[19]	;	# color of print values for each box (black if undef)
	my	$sref_boxtextsize	=	$_[20]	;	# size of print values for each box (size = "4" if undef)
	my	$sref_boxlineactive	=	$_[21]	;	# if value == 1 -> boxlines activated (with default or further specified setup)
	my	$sref_boxlinecolor	=	$_[22]	;	# color of each box line ("white" if undef)
	my	$sref_boxlinesize	=	$_[23]	;	# witdh of each box line (size = "1.5" if undef)
	my	$sref_scalefilln	=	$_[24]	;	# number of colors passing the defined color palette; "scale_fill_distiller" used instead of "scale_fill_gradientn" if undef
	my	$sref_title			=	$_[25]	;	# grafic title line
	my	$sref_legendactive	=	$_[26]	;	# if value == 1 -> activation of legend customization; if value == 2 -> no legend
	my	$sref_legendwidth	=	$_[27]	;	# legend width, only active if $legendactive == 1
	my	$sref_legendheight	=	$_[28]	;	# legend height, only active if $legendactive == 1
	my	$sref_legendtitle	=	$_[29]	;	# legent title, independent of $legendactive
	
	######################################################################
	## print heatmap plot using R                                       ##
	######################################################################
	...
	#########################################
	# storing individual command lines
	my	@command_lines ;
	#########################################
	...
	######## GG TITLE COMMAND LINE ###################################################
	##################################################################################
	if ( $$sref_title ){ push @command_lines, "ggtitle(\"".$$sref_title."\")" }
	##################################################################################
	##################################################################################
	
	######## GEOM_TILE ###############################################################
	##################################################################################
	if ( $$sref_boxlineactive ){
		
		# do boxlines
		unless ( $$sref_boxlinecolor ){ $$sref_boxlinecolor = "white" }
		unless ( $$sref_boxlinesize  ){ $$sref_boxlinesize  = 1.5 	  }
		push @command_lines, "geom_tile(color = \"$$sref_boxlinecolor\", lwd = $$sref_boxlinesize, linetype = 1)"
	}
	# without boxlines
	else{ push @command_lines, "geom_tile()" }
	##################################################################################
	##################################################################################
	
	######## COORD_FIX ###############################################################
	##################################################################################
	if ( $$sref_coord_fix == 1 ){ push @command_lines, "coord_fixed()" }
	##################################################################################
	##################################################################################
	
	######## GEOM_TEXT ###############################################################
	##################################################################################
	unless ( $$sref_boxtextcolor ){ $$sref_boxtextcolor = "black" }
	unless ( $$sref_boxtextsize  ){ $$sref_boxtextsize  = 5 	  }
	if ( $$sref_geomtext == 1 ){ push @command_lines, "geom_text(aes(label = $$sref_boxtext), color = \"$$sref_boxtextcolor\", size = $$sref_boxtextsize)" }
	##################################################################################
	##################################################################################
	
	######## SCALE FILL COMMAND LINE #################################################
	##################################################################################
	# boxcolors either following  "scale_fill_gradientn" or ""scale_fill_distiller" sheme
	# "scale_fill_gradientn" if defined $$sref_scalefilln (number of color scales, e.g. $$sref_scalefilln = 20)
	# "scale_fill_distiller" if undef $$sref_scalefilln
	my %allowed_fillcolor =	(
								'YlOrRd'	=> 1 , 'PuBuGn'		=> 1 , 'BuGn'		=> 1 , 'Dark2'		=> 1 , 'PRGn'	=> 1 ,
								'YlOrBr'	=> 1 , 'PuBu'		=> 1 , 'Blues'		=> 1 , 'Accent'		=> 1 , 'PiYG'	=> 1 ,
								'YlGnBu'	=> 1 , 'OrRd'		=> 1 , 'Set3'		=> 1 , 'Spectral'	=> 1 , 'BrBG'	=> 1 ,
								'YlGn'		=> 1 , 'Oranges'	=> 1 , 'Set2'		=> 1 , 'RdYlGn'		=> 1 ,
								'Reds'		=> 1 , 'Greys'		=> 1 , 'Set1'		=> 1 , 'RdYlBu'		=> 1 ,
								'RdPu'		=> 1 , 'Greens'		=> 1 , 'Pastel2'	=> 1 , 'RdGy'		=> 1 ,
								'Purples'	=> 1 , 'GnBu'		=> 1 , 'Pastel1'	=> 1 , 'RdBu'		=> 1 ,
								'PuRd'		=> 1 , 'BuPu'		=> 1 , 'Paired'		=> 1 , 'PuOr'		=> 1 ,
	) ;
	
	if 	  ( ( $$sref_scalefilln ) && ( $allowed_fillcolor{$$sref_fillcolor} ) ) { push @command_lines, "scale_fill_gradientn(colors = hcl.colors(".$$sref_scalefilln.", \"".$$sref_fillcolor."\"))" }
	elsif 	( $$sref_scalefilln ) { push @command_lines, "scale_fill_gradientn(colors = hcl.colors(".$$sref_scalefilln.", \"Spectral\"))"  }
	elsif 	( $$sref_fillcolor  ) { push @command_lines, "scale_fill_distiller(palette = \"".$$sref_fillcolor."\")"		}
	else 						  { push @command_lines, "scale_fill_distiller(palette = \"Spectral\")" 				}
	#push @command_lines, "scale_fill_gradient(low=\"white\", high=\"blue\")" ;
	##################################################################################
	##################################################################################
	
	######## Legend Customization ####################################################
	##################################################################################
	if ( $$sref_legendactive ){ 
		
		# customize title, width and height of the legend
		if ( $$sref_legendactive == 1 ){
			
			unless ( $$sref_legendwidth  ){ $$sref_legendwidth  = 0.5 }
			unless ( $$sref_legendheight ){ $$sref_legendheight = 20  }
			
			if ( $$sref_legendtitle ){ push @command_lines, "guides(fill = guide_colourbar(title = \"".$$sref_legendtitle."\", barwidth = ".$$sref_legendwidth.", barheight = ".$$sref_legendheight."))" }
			else 					 { push @command_lines, "guides(fill = guide_colourbar(barwidth = ".$$sref_legendwidth.", barheight = ".$$sref_legendheight."))" }
		}
		
		# remove legend
		elsif ( $$sref_legendactive == 2 ){ push @command_lines, "theme(legend.position = \"none\")" }
	}
	
	# customize legend title
	elsif ( $$sref_legendtitle  ){ push @command_lines, "guides(fill = guide_colourbar(title = \"".$$sref_legendtitle."\"))" }
	##################################################################################
	##################################################################################
	...
	if	( $$sref_flag_facet ){
		...
		#########################################
		# change x,y scale command of each panel from fixed (default) to defined parameter ($$sref_scale_facet)
		if	( $$sref_scale_facet ){ if ( $$sref_scale_facet =~ /free|free_x|free_y/ ){	push @parameter, "scales=\"".$$sref_scale_facet."\"" }}
		#########################################
	...
	######## Theme Axis Text #########################################################
	##################################################################################
	if ( $$sref_rotate_xlabel ){ push @theme_lines, "axis.text.x = element_text(color=\"black\", size=18, angle=".$$sref_rotate_xlabel.", hjust = 1)"	}
	else					   { push @theme_lines, "axis.text.x = element_text(color=\"black\", size=18)" 	}
	push @theme_lines, "axis.text.y = element_text(color=\"black\", size=18)" ;
	##################################################################################
	##################################################################################
...
sub read_in_p4_results{
	...
	while ( my $line = <IN> ){
		...
		if	( $line =~ /Bad/ ){ unless ($line =~ /Bad newt.  Likelihood decreased./){ die "\nP4-ERROR:\n", $line, "!\n\n" }}
	...
	for my $line ( @$aref_l_qcalcfile ){
		...
		# check correct clan association of each of the four taxa in spd file
			...
			if ( $href_clan_of_seq->{$prts[1]} ne $prts[2] ){
				...
				die "\nSPDFILE-ERROR: Incongruent defined clade '", $prts[2] ,"' of '", $prts[1] , "' in ", $$sref_p_qcalc_file, "!\n\n" ;
			}
		}
		#########################################
	}
...
sub rooted_clade_support_summary{ # added 15.04.25
	...
	my	$href_subfolder_q						= $_[0]	; # key1: R grafic unique printcode (e.g. 'metafileQ1'); key2: *.tsv filename; value: *tsv filename data addings 			IN: defined; OUT: unchanged
	my	$href_hoh_data_of_tsvfile_of_prcode		= $_[1]	; # key: output-type (e.g. 'pdf'); value: pathdir																			IN: defined; OUT: changed
	my	$href_value_of_scr_par					= $_[2]	; # key1: sealion user available parameter options; value: optional parameter setting										IN: defined; OUT: unchanged
	my	$href_seen_rctrees_of_ts				= $_[3] ; # key1: average (mean|median); key2: newick of best three trees; value: counter best three trees							IN: defined; OUT: unchanged
	my	$sref_clanfile							= $_[4] ; # name of given clanfile																									IN: defined; OUT: unchanged
	
	######################################################################
	# if additional rooted-clade tree file is optionally specified,
	# read IN defined newick strings and add these trees to the sampling of sealion single analysis identified best three trees
	if ( $href_value_of_scr_par->{sys_parameter}{trsearch} ){
		
		## READ INFILE
		my $treedef_file = $href_value_of_scr_par->{sys_parameter}{trsearch} ;
		my @def_trees 	 = &readIN_file ( \$treedef_file ) ;
		
		## ADD trees to the overall single analysis best-three tree samplings
		my @averages = keys %$href_seen_rctrees_of_ts ;
		
		for my $aver ( @averages ){
			
			for my $def_tree ( @def_trees ){ $href_seen_rctrees_of_ts->{$aver}{$def_tree}++ }
		}
	}
	######################################################################
	...
	######################################################################
	#
	my $pdir = $href_subfolder_q->{mcm} ;
	for my $iceresult (<${pdir}/*all_topology_scores.txt>){ 
		
		print "\n\tsampling single rooted-clade tree scores from ", $iceresult ;
		
		## sampling of icebreaker tree support files
		my @def_trees = &readIN_file ( \$iceresult ) ;
		
		## extract best tree support (first line)
		my @besttree_info 	= split "\t", $def_trees[0] ;
		my $s_besttree		= $besttree_info[1] ;
		
		## sampling of single trees, support, and ranking
		my %s_of_t ;	# key: tree; value: support
		my %p_of_t ;	# key: tree; value: ranking of support
		for my $line ( 0 .. @def_trees-1 ){
			
			my @prts = split "\t", $def_trees[$line] ;
			$s_of_t{$prts[0]} = $prts[1] ;
			$p_of_t{$prts[0]} = $line+1  ;
		}
		
		## etract average measure info from filename
		my $ts ;
		$iceresult =~ s/^$pdir\/|_all_topology_scores.txt$//g ;
		if 		( $iceresult =~ /_mean_nap/   ){ $ts = "mean"   ; $iceresult =~ s/^.*_mean_nap/nap/g   }
		elsif 	( $iceresult =~ /_median_nap/ ){ $ts = "median" ; $iceresult =~ s/^.*_median_nap/nap/g }
		else{ die "\nBUG-ERROR: Cannot assign average measure in subroutine &rooted_clade_support_summary!\nPlease, report BUG to software developer!\n\n" }
		#print "\n\tsampling trees from ", $iceresult ;
		
		## extract support measure (nap, napr, napv: first array-str)
		my @prts = split "_", $iceresult ;
		my $cm	 = $prts[0] ;
		
		## reject outgroup code (last element)
		pop @prts ;
		my $cm_fm = join "_", @prts ;
		
		## recoding of method code for print out
		my $recoded_fm = &recode_filter_printout( \$cm_fm ) ;
		#print "\n\tfm ", $recoded_fm ;
		
		
		&print_tsv_MQ7(
			
			\%$href_subfolder_q						,	# key1: R grafic unique printcode (e.g. 'metafileQ1'); key2: *.tsv filename; value: *tsv filename data addings 			IN: defined; OUT: unchanged
			\%$href_hoh_data_of_tsvfile_of_prcode	,	# key: output-type (e.g. 'pdf'); value: pathdir																			IN: defined; OUT: changed
			\%$href_seen_rctrees_of_ts				,	# key1: average (mean|median); key2: newick trees; value: counter best three trees										IN: defined; OUT: unchanged
			\%s_of_t								,	# key1: mean|median; key2: tree; value: support																			IN: defined; OUT: unchanged
			\%p_of_t								,	# key1: mean|median; key2: tree; value: support ranking																	IN: defined; OUT: unchanged
			\$recoded_fm							,	# method code for print out																								IN: defined; OUT: unchanged
			\$cm									,	# actual method code (nap, napr, napv)																					IN: defined; OUT: unchanged
			\$ts									,	# support type: mean|median																								IN: defined; OUT: unchanged
			\$s_besttree							,	# support of best file tree																								IN: defined; OUT: unchanged
...
sub start_p4{
	...
	my		$program			=	"p4" ;
			$$sref_outfile_p4	=	"p4_result_".$$sref_alignment."_".$$sref_clan4."_q".$$sref_qcounter.".txt" ;
...
sub treefishing_result_processing{ # changed 15.04.25
	...
	my	$href_seen_rctrees_of_ts				= $_[9] ; # key1: average (mean|median); key2: newick of best three trees; value: counter best three trees							IN: undefined; OUT: defined/changed
	...
	for my $iceresult (<${$sref_outfile_prefix}_*top_topology_scores.txt>){ 
		...
		####################### added 15.04.25
		# global sampling of best-three trees, separated by average measure
		for my $bestthree_tree (@trees){ $href_seen_rctrees_of_ts->{$ts}{$bestthree_tree}++ }
		#######################
...
sub write_p4_script{
	...
	# print script code
	$$sref_script_p4	.= "_p4_temp_script.py" ;
